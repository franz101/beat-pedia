...
const sampleMap = {
    "BD": "./samples/808/BD/BD0000.WAV",
    "SD": "./samples/808/SD/SD0000.WAV",
    "CP": "./samples/808/CP/CP.WAV",
    "HT": "./samples/808/HT/HT00.WAV",
    "CH": "./samples/808/CH/CH.WAV",
    "OH": "./samples/808/OH/OH00.WAV",
    "CY": "./samples/808/CY/CY0000.WAV"
}
const loadedSamples = {};
...
Promise.all(Object.values(sampleMap).map(loadSample)).then((samples) => {
    Object.keys(sampleMap).forEach((key, i) => {
      loadedSamples[key] = samples[i];
    });
  });

  
const getGeneratedSample = (name) => {
    const generateSample = (context) => {
        const sampleSource = context.createBufferSource();
            sampleSource.buffer = loadedSamples[name];
            sampleSource.connect(context.destination);
            sampleSource.start(context.currentTime);
    };
    return generateSample
  };

// New generateSnare function using the loaded sample without any filters
const generateCHSample = getGeneratedSample('CH');
const generateSnareSample = getGeneratedSample('SD');
const generateOHSample = getGeneratedSample('OH');
const generateClapSample = getGeneratedSample('CP');
const generateCYSample = getGeneratedSample('CY');
export {
    generateKick,
    generateSnare,
    generateSnareSample,
